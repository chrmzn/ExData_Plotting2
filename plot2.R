...
summarise_data <- function(df){
    df %>% 
        ...
        # and to keep both graphs (Plot1/Plot2) on the same scale'ish
        mutate(total_emissions=total_emissions/1000)
}

plot2 <- function( summary_data ){
    # Create our expression label and total
    plotYLabel <- expression(PM[2.5] * " Emissions (1,000's of Tons)" )
    plotTitle <- expression("Total Baltimore City " * PM[2.5] * " Emissions")
    # Setup the plot
...
plotGraph('plot2.png', plot2, data_summary)